...
MediaMovie::MediaMovie(Meta item) : itemId(item.getId()) {
...
void MediaMovie::doMovie() {
    ...
    for (const auto& addon : addons) {
        Meta meta = addon.getMeta("movie", this->itemId);
        if (meta.getId() == this->itemId) {
            this->headerTitle->setTitle(meta.getName());
            this->labelYear->setText(meta.getYear());
            this->labelOverview->setText(meta.getDescription());

            this->parentalRating->getParent()->setVisibility(brls::Visibility::GONE);
            this->people->setVisibility(brls::Visibility::GONE);

            if (meta.getGenres().empty()) {
                this->labelGenres->setVisibility(brls::Visibility::GONE);
            } else {
                this->labelGenres->setText(fmt::format("{}", fmt::join(meta.getGenres(), ", ")));
                this->labelGenres->setVisibility(brls::Visibility::VISIBLE);
            }

            if (!meta.getLogoUrl().empty()) {
                Image::load(this->imageLogo, meta.getLogoUrl());
                this->imageLogo->setVisibility(brls::Visibility::VISIBLE);
            }
